...
- `fix-plan.md`: epics → tickets (each ticket points at its problem file) **plus the execution plan**: agent bundles, lanes, and order (next section's rules). **The plan is a teaching document, not an index (founder standard, 2026-08-18):** every epic opens with the story of what happened on the audited run and why the group exists; every ticket is self-contained in five labeled parts — **What is going wrong** (observed behavior, with the incident's concrete example), **What should happen** (correct behavior), **Why it is happening** (root cause explained conceptually), **The fix** (what will change, explained in concepts BEFORE any file path appears), **Outcome** (what a reader can verify is different after). A reader with none of the audit's context — the founder, a developer, a fresh agent — must understand the problem and the proposal from the ticket alone; file:line and problem-file pointers close a ticket as implementer references, never substitute for the explanation. No shorthand, no flag/code names without their plain-language meaning, no "the F1 defect class"-style internal labels. Present in chat: findings story → plan → lanes. **STOP for the requester's feedback and agreement. Nothing is built before explicit approval — and a fix that adds a new subsystem or a new refusal path needs FOUNDER sign-off per D9/D10, whoever requested the audit.**
